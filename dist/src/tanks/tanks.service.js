...
let TanksService = class TanksService {
    ...
    async getFifoSuggest() {
        ...
        if (tanks.length === 0) {
            throw new common_1.NotFoundException('ไม่มีถังที่มีหมูอยู่');
        }
    ...
    async fill(tankId, dto, actorId) {
        const tank = await this.prisma.tank.findUnique({ where: { id: tankId } });
        if (!tank)
            throw new common_1.NotFoundException(`ไม่พบถัง id: ${tankId}`);
        const thawEvent = await this.prisma.thawEvent.findUnique({
            where: { id: dto.thawEventId },
            include: { lot: true },
        });
        if (!thawEvent)
            throw new common_1.NotFoundException(`ไม่พบ ThawEvent id: ${dto.thawEventId}`);
        if (thawEvent.status !== 'WAITING_TANK') {
            throw new common_1.BadRequestException(`ThawEvent ยังไม่พร้อมเข้าถัง (status: ${thawEvent.status})`);
        }
        if (dto.weightKg > thawEvent.remainingKg) {
            throw new common_1.BadRequestException(`น้ำหนักที่ใส่ (${dto.weightKg}) มากกว่าคงเหลือใน ThawEvent (${thawEvent.remainingKg})`);
        }
        const newWeight = tank.currentWeightKg + dto.weightKg;
        if (newWeight > tank.capacityKg) {
            throw new common_1.BadRequestException(`น้ำหนักเกิน capacity (${newWeight} > ${tank.capacityKg} กก.)`);
        }
    ...
    async withdraw(tankId, dto, actorId) {
        ...
        if (tank.currentWeightKg <= 0) {
            throw new common_1.BadRequestException(`ถัง ${tank.tankNumber} ว่างอยู่`);
        }
        if (dto.weightKg > tank.currentWeightKg) {
            throw new common_1.BadRequestException(`น้ำหนักที่เบิก (${dto.weightKg}) มากกว่าที่มีในถัง (${tank.currentWeightKg})`);
        }
        const newWeight = tank.currentWeightKg - dto.weightKg;
    ...
    async withdrawAuto(dto, actorId) {
        const tanks = await this.prisma.tank.findMany({
            where: { currentWeightKg: { gt: 0 } },
            include: {
                tankEntries: {
        ...
        if (dto.weightKg > totalAvailable) {
            throw new common_1.BadRequestException(`สต็อกในถังไม่พอ — มีทั้งหมด ${totalAvailable} กก. ต้องการ ${dto.weightKg} กก.`);
        }
        const plan = [];
